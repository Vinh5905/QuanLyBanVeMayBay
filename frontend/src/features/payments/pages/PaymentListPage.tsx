...
export function PaymentListPage() {
  ...
  return (
    <div className="payment-list-page">
      ...
      {!loading && !error && payments.length === 0 && <EmptyState title="Không có hóa đơn" description="Chưa có hóa đơn nào" />}
      ...
      <Modal open={!!selectedPayment} onClose={() => setSelectedPayment(null)} title="Chi tiết hóa đơn">
        {selectedPayment && (
          <div className="payment-detail">
            <p><strong>Mã hóa đơn:</strong> {selectedPayment.maThanhToan}</p>
            <p><strong>Mã vé:</strong> {selectedPayment.maVe ?? 'N/A'}</p>
            <p><strong>Số tiền:</strong> {(selectedPayment.soTien || 0).toLocaleString('vi-VN')}đ</p>
            <p><strong>Thuế VAT:</strong> {((selectedPayment.thueVAT || 0)).toLocaleString('vi-VN')}đ</p>
            <p><strong>Tổng thanh toán:</strong> <strong>{((selectedPayment.soTien || 0) + (selectedPayment.thueVAT || 0)).toLocaleString('vi-VN')}đ</strong></p>
            <p><strong>Phương thức:</strong> {selectedPayment.phuongThuc}</p>
            <p><strong>Trạng thái:</strong> {selectedPayment.trangThaiThanhToan}</p>
            <p><strong>Ngày thanh toán:</strong> {selectedPayment.thoiGianThanhToan ? new Date(selectedPayment.thoiGianThanhToan).toLocaleString('vi-VN') : ''}</p>
          </div>
        )}
      </Modal>
    </div>
